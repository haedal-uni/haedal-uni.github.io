...
공통적으로 stay 의도(`파라를 하고싶어`, `애쉬로 계속 하고 싶은데`)와 

counter 맥락(`솔저랑 소전`, `둠피때문에`)이 한 문장에 섞여 있었고
...
[issue](https://github.com/haedal-uni/overwatch/issues/1)
...
"이기고싶어"만 들어온 경우 특정 영웅을 유지하겠다는 의도가 아닐 수 있으므로  

영웅 이름이 있을 때만 `stay`로 분류한다.
...
`merge_context_node`에서 

`stay`, `performance_improve` 의도일 때도 이전 대화의 `target_enemy`를 이어받도록 로직을 추가했다.
...
- **해결**: `detect_wants_to_keep_hero()`로 stay 표현을 먼저 확인하고

  `True`이면 다른 키워드보다 우선해서 `stay`로 분류한다.
