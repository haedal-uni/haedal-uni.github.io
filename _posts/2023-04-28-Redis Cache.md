...
#### Key Serializer와 Value Serializer 설정
...
```
위 코드는 Redis의 다양한 데이터 구조(예: Hash, List, Set, Sorted Set 등)를 지원한다.

참고로 아래와 같이 작성할 수도 있다.
...
```
StringRedisTemplate은 기본적으로 키와 값을 문자열(String)로만 다룰 때 최적화된 클래스다.

<br><br>

Redis의 자료 구조 혹은 확장 가능성을 고려한다면 RedisTemplate<String, String>로 사용을 하는 것이 좋고

문자열만 다루는 경우 StringRedisTemplate를 적용하면 좋을 것 같다.
