...
for category in data["독립변수"].unique():
    subset = data[data["독립변수"] == category]
    q1 = subset["종속변수"].quantile(0.25)
    q3 = subset["종속변수"].quantile(0.75)
    iqr = q3 - q1
    ...
    # 기준을 벗어난 값(이상치) 제거
    filtered_data = data[~((data["독립변수"] == category) & 
                          ((data["종속변수"] < lower_bound) | 
                           (data["종속변수"] > upper_bound)))]
...
sns.boxplot(x="독립변수", y="종속변수", hue="색상 구분을 위한 추가범주",
 data=data,  palette="coolwarm", legend=False)

plt.xlabel("독립변수")
plt.ylabel("종속변수")
plt.title("Distribution of 종속변수 by 독립변수")  
plt.show()
```
...
sns.lmplot(x='독립변수', y='종속변수', data=data)
plt.show()
```
...
```py
# cut을 사용한 범주화
data['종속변수_cut'] = pd.cut(data['종속변수'], bins=[60, 80, 90, 100], labels=['낮음', '중간', '높음'])
data['독립변수_cut'] = pd.cut(data['독립변수'], bins=[50, 65, 75, 100], labels=['낮음', '중간', '높음'])

# qcut을 사용한 범주화
data["attendance_cut"] = pd.qcut(data["종속변수"], q=3, labels=['낮음', '중간', '높음'])
data["독립변수_cut"] = pd.qcut(data["독립변수"], q=3, labels=['낮음', '중간', '높음'])
```
...
```py
import scipy.stats as stats
statis_x, p_x = stats.shapiro(data["독립변수"])
statis_y, p_y = stats.shapiro(data["종속변수"])
...
statis_x, p_x = stats.kstest(data["독립변수"], 'norm')
statis_y, p_y = stats.kstest(data["종속변수"], 'norm')
...
spearman_corr, spearman_p = stats.spearmanr(data['독립변수'], data['종속변수'])
print(f"Spearman 상관계수: {spearman_corr}, p-value: {spearman_p}")

# 3개 이상
vars = data[['독립변수 1', '독립변수 2', '종속변수']]
...
data_X = data['Exam_Score'] 
y = data['Attendance']  # 종속 변수
...
formula = "종속변수 ~ 독립변수1 + 독립변수2 + C(범주형 변수)"
