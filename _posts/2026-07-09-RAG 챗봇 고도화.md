...
`None`이 들어오면 세션에 저장된 이전 선택값을 쓰고 없다면 `"detailed"`(자세히)를 기본값으로 설정했다. 
...
수치가 다른 스탯 질문이나 다른 영웅을 언급한 경우에는 그래프를 실행한다.
...
우리팀 정보가 있으면 답변 프롬프트에도 포함해서 시너지를 고려한 조언이 가능하다.
...
상대팀 캡처 그룹이 "우리팀" 마커까지 삼켜버려 우리팀 영웅이 상대팀에 섞이는 문제가 있었다.
...
상대팀을 추출할 때는 우리팀 마커(`우리팀`, `우리팀`) 이전까지만 잘라서 사용하고, 우리팀 추출할 때는 반대로 적용한다.
...
| 조합 인식 | 우리팀 조합 추출, 교차 오염 방지, 역할 추론 |
| 영웅 별칭 | 줄임말/오타 7개 추가 |
