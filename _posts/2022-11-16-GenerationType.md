...
GenerationType 내용을 작성하게 된 이유는 연관관계 매핑 test 코드를 실행하면서 생긴 문제들이 GenerationType과 관련되어있기 때문이다.   
...
*[연관관계 - 이론 정리](https://haedal-uni.github.io/posts/%EC%97%B0%EA%B4%80%EA%B4%80%EA%B3%84/)                
...
연관관계 test 코드를 실행시키는데 pk를 1L로 저장을 하고 꺼내올 때 

1L로 꺼내오려고 하면 에러가 뜨고 계속 2L로 가져오는 상황이 나타났다.
...
그래서 그 이유가 무엇인지 파악 하기 위해서 처음에는 IDENTITY와 AUTO에 대해서 비교하는 글을 쓰려고 했다.

그러던 중 아래와 같이 test를 해보면서 IDENTITY와 AUTO가 아닌 GenerationType으로 글을 쓰게 되었다.
...
매핑된 id값이 0이거나 매핑되어있는 id의 자식객체가 없을 때 오류가 발생하는 경우가 있다.

이 경우 매핑되는 애들이 없을 때 값을 null 처리를 하게되면 아래와 같이 조회를 할 수 있다.         
...
`IDENTITY`로 설정 후 3L로 저장했을 때 3L을 갖고오지 못하는 이유는 auto_increment로 인해 1L로 저장이 된다.
...
여기서 몇가지를 파악할 수 있었다. `AUTO`로 설정하면
1. test 코드에서 Current value를 서로 공유하는 것을 볼 수 있었다.       
2. application으로 실행하면 Syestem_Sequence가 떴고 test에서는 table hibernate_sequences가 생성되었다.
