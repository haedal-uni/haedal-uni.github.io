...
실제 `quiz` 테이블은 건드리지 않으려고 벤치마크 전용 `quiz_bench` 테이블을 만들어 

오답 데이터를 100 / 1,000 / 10,000 / 100,000개씩 채운 뒤 측정했다. (측정 후 테이블은 삭제한다.)
...
- Java에서 순번을 뽑는 ③번 방식은 모든 구간에서 제일 느렸다.
  ...
  쿼리를 두 번 날리는 데다(`count` + `IN`), `1 ~ total` 리스트를 통째로 만들어 섞기 때문이다.

  100,000개에서 168ms까지 갔다.
...
- Java에서 순번을 뽑는 방식은 "항상 10개(부족하면 전부) + 확실히 섞임"을 보장할 수 있다는 장점이 있다.
  ...
  단, 데이터가 커지면 `1 ~ total` 리스트를 만드는 비용이 붙으니 오답이 많아진다면 이 부분은 다시 봐야 한다. 
