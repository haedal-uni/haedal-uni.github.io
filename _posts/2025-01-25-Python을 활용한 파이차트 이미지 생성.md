...
`count()`는 중복을 포함하지만 Study 테이블에서 

`UNIQUE user_id (user_id, word_id, meaning_id, sentence_id)`로 한 사용자에 대해서는 중복이 될 수 없다. 

따라서 DISTINCT를 적용하지 않아도 문제 없이 동작하므로 적용하지 않았다.
