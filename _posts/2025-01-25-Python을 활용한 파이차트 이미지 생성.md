...
![AC_ 20250123-014953](/assets/img/posts/64c56c35-129c-4fea-b798-f1ab49705744.gif)
...
study = [level_0, level_1, level_2]
level = ['Easy', 'Medium', 'Hard']
explode = [0.03, 0.03, 0.03]
colors = ['#FF9999', '#FFCC99', '#99CCFF'] # 색상 지정 

plt.pie(study, labels=level, autopct='%.1f%%',
        startangle=90, counterclock=False,
        colors=colors, explode=explode, shadow=True)
plt.show()
```
- `autopct` : 표시 형식 → 소수점 한자리(`.1f`)

- `startangle` : 시작 각도(default : 0, 반시계)  

- `counterclock` : 시계방향 (default : True, 반시계)
  
- `explode` : 분리 → 3% 만큼 분리 

- `shadow = True` : 그림자

![image](/assets/img/posts/bff31aab-3141-4b98-a011-7797088cb353.png)


<br><br>

### 이미지 저장 및 반환
`savefig()` : 그래프를 이미지 파일로 저장한다.
```py
import io
