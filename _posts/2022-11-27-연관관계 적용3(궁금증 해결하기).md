...
바로 `getOne()`을 사용하는 것인데 spring boot version 2.5 미만의 경우 `getOne()`을 사용하고
...
#### `findById()`와 `getReferenceById(ID)`
...
만약 아래 처럼 에러가 뜬다면 `findById()`를 쓰면 된다.
...
지금은 `getReferenceById()`를 이용해서 comment에 넣어준다.
