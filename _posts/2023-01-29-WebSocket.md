...
위 img는 handleTransportError가 포함된 AbstractWebSocketHandler 추상클래스다. 

패키지명이 WebSocketSession과 같다.
...
Message의 Payload는 sendMessage() 메서드가 반환한 값을 기반으로 하고, 기본적으로 Destination 헤더는 `/topic/public`으로 설정된다.
...
imple In-Memory Broker를 사용하도록 설정.
