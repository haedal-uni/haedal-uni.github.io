...
하지만 이렇게 역직렬화의 타입을 Object로 일괄적으로 적용할 시에는 

배열 또는 컬렉션 타입의 데이터를 역직렬화하지 못하는 문제가 발생했다.  

그 이유는 DefaultTyping 옵션 때문이다. 

<br>

일반적으로 Jackson은 ObjectMapper의 readValue 메서드에 전달된 타입 정보를 사용하여 데이터를 역직렬화한다.  

그러나 역직렬화 타입이 Object로 지정되면 

JSON에 포함된 데이터의 구체적인 타입 정보를 Jackson이 알지 못해 역직렬화가 불가능하게되는 문제가 발생한다.

<br>

Jackson은 DefaultTyping을 통해 JSON에 타입 정보를 포함시킬 수 있다.  

GenericJackson2JsonRedisSerializer는 타입 정보를 JSON에 포함하지만 

명시적으로 `@class` 속성을 추가하지는 않는다.

이로 인해 역직렬화 시 타입을 명확히 알 수 없는 경우 문제가 발생할 수 있다.

<br>

`JsonTypeInfo.As.PROPERTY`를 사용하면 JSON 내부에 타입 정보를 속성으로 포함시킬 수 있어

아래와 같이 `activateDefaultTyping`를 Redis Config에 추가했다.       
```java
objectMapper.activateDefaultTyping(
            objectMapper.getPolymorphicTypeValidator(),  // 안전한 타입 검증 활성화  
...
다양한 클래스 타입을 JSON 형태로 저장할 수 있는 범용 Serializer로   

타입 정보를 JSON에 포함하기 위해 내부적으로 defaultTyping을 활성화한다.

<br>

*DefaultTyping 옵션 : JSON에 타입 정보를 저장할 범위를 설정 


<br><br>   

참고로 `enableDefaultTyping`은 더 이상 사용하지 않으므로 `activateDefaultTyping`로 작성했다.     
