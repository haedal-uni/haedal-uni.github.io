...
![image](/assets/img/posts/7f687b66-4534-45c9-a9f7-bff172c7bf8e.png){: width="50%"}   

현재 기본적인 구조는 영어 단어 학습과, 빈칸 채우기 퀴즈, 그리고 틀린 부분을 모아서 복습하는 사이트다.

<br>

![AC_ 20241030-190759](/assets/img/posts/b12db838-8fb4-46fb-a5ed-3e6dd056ba5d.gif){: width="50%"}   
...
![image](/assets/img/posts/e2e3ab9c-c413-4a92-bff6-071623ac88de.png)
...
![image](/assets/img/posts/62f1f55c-1ae9-48b6-85be-4eca4f6ad1c1.png)
...
public interface MeanRepository extends JpaRepository<Meaning, Long> {
    int existsByWordIdAndMeaning(Long wordId, String meaning);
...
![image](/assets/img/posts/a93c622b-ffd4-4273-b518-6df810ad9203.png)
...
public void saveMeaning(MultipartFile file) throws IOException {
    ...
    while (rowIterator.hasNext()) {
        ...
        if(!Objects.requireNonNull(map.get(wordText)).contains(meaningText)){
            ...
            int exists = meanRepository.existsByWordIdAndMeaning(word.getId(), meaningText);
            if (exists==0) {
